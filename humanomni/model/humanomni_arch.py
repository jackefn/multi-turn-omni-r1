...
class HumanOmniMetaModel:
    ...
    def __init__(self, config):
        ...
        num_branches = 3
        # 使用HuggingFace Hub模型ID或本地路径
        # 优先尝试本地路径，如果不存在则使用HuggingFace Hub模型ID
        local_bert_path = "/data1/models/bert-base-uncased"
        if os.path.exists(local_bert_path):
            bert_model = local_bert_path
        else:
            bert_model = "bert-base-uncased"  # 使用HuggingFace Hub模型ID
       ...
       # self.feature_compressor = SFDynamicCompressor(config, self.vision_tower)
        ...
        if hasattr(config, "mm_audio_tower") and config.mm_audio_tower:
