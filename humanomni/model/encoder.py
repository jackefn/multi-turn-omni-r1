...
def build_audio_tower(audio_tower_cfg, **kwargs):
    ...
    # 检查路径或模型ID中是否包含whisper（不区分大小写）
    audio_tower_lower = str(audio_tower).lower()
    if "whisper" in audio_tower_lower:
        try:
            return WhisperAudioTower(audio_tower, args=audio_tower_cfg, **kwargs)
        except Exception as e:
            print(f"Warning: Failed to build audio tower from {audio_tower}: {e}")
            return None
    else:
