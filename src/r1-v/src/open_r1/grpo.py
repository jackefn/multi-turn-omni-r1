...
class GRPOScriptArguments(ScriptArguments):
    ...
    # Override dataset_name to make it optional when using json_dataset_path
    dataset_name: Optional[str] = field(
        default=None,
        metadata={"help": "Dataset name (optional if json_dataset_path is provided)"},
    )
    ...
    json_dataset_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to JSON dataset file for video training"},
    )
    data_folder: Optional[str] = field(
        default=None,
        metadata={"help": "Base folder path for resolving relative video paths in dataset"},
    )
...
def main(script_args, training_args, model_args):
    ...
    # Load the dataset
    if script_args.json_dataset_path:
        json_file_path = script_args.json_dataset_path
    elif script_args.dataset_name:
        # Fallback to dataset_name if json_dataset_path is not provided
        json_file_path = script_args.dataset_name
    else:
        raise ValueError("Either --json_dataset_path or --dataset_name must be provided")
    
    dataset = load_video_dataset(json_file_path)
    
    # If data_folder is provided, update video paths to be absolute
    if script_args.data_folder:
        def update_video_paths(example):
            video_path = example["video"]
            if not os.path.isabs(video_path):
                # Make path absolute by joining with data_folder
                example["video"] = os.path.join(script_args.data_folder, video_path)
            return example
        
        dataset = dataset.map(update_video_paths)
