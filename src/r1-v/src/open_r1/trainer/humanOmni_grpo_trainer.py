...
# 必须在导入humanomni之前添加路径！
# 动态添加humanomni模块路径（相对于当前文件位置）
# 当前文件: src/r1-v/src/open_r1/trainer/humanOmni_grpo_trainer.py
# humanomni目录: ../../../../../humanomni (从src/r1-v/src/open_r1/trainer/向上5级到R1-Omni根目录)
current_file_dir = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.abspath(os.path.join(current_file_dir, '../../../../../'))
humanomni_path = os.path.join(project_root, 'humanomni')
if os.path.exists(humanomni_path) and project_root not in sys.path:
    sys.path.insert(0, project_root)
# 保留旧的路径作为备选（如果存在）
old_humanomni_path = '/data1/xiangc/mxy/HumanOmni/'
if os.path.exists(old_humanomni_path) and old_humanomni_path not in sys.path:
    sys.path.append(old_humanomni_path)
old_hf_path = '/data1/xiangc/mxy/huggingface/'
if os.path.exists(old_hf_path) and old_hf_path not in sys.path:
    sys.path.append(old_hf_path)

# 现在可以安全地导入humanomni模块了
    ...
    def __init__(
       ...
       # import ipdb;ipdb.set_trace()
        ...
        if isinstance(model, str):
            ...
            config.mm_vision_tower = '/data1/models/siglip-base-patch16-224'
            config.mm_audio_tower = '/data1/models/Whisper-large-v3'
        ...
        bert_model = "/data1/models/bert-base-uncased"
        self.bert_tokenizer = BertTokenizer.from_pretrained(bert_model)


        # Processing class
        if processing_class is None:
            processing_class = processing_class = AutoProcessor.from_pretrained("/data1/models/Qwen2-VL-2B-Instruct")
